function [] = recon_phantom(slices,sens_lowres, sens_highres, kdata_folder)
    experiments = [1:6, 8];
    ratios = [...
                gr2D,...
                1/46,...
                ...
                SILVER_2D([8,55], 'Winkelmann'),...
                SILVER_2D([10,46], 'electrostatic_potential'),...
                SILVER_2D([10,46], 'Winkelmann')];


    window_sizes_per_exp = {[8,10,46,55],46,55,10,8,[8,55],[8,55],[10,46],[10,46]};
    ...
    for ex = experiments
        ...
        for sl = slices
            kdata_file_a = dir([kdata_folder 'meas_*' num2str(ex) 'a*slice' num2str(sl) '.mat']);
            kdata_file_b = dir([kdata_folder 'meas_*' num2str(ex) 'b*slice' num2str(sl) '.mat']);
            ...
            for ws = window_sizes
                ...
                recon_a = reshape(E.iter(kdata_rs_a,@pcg,[],100), mat_size, mat_size, 1, []);
                recon_b = reshape(E.iter(kdata_rs_b,@pcg,[],100), mat_size, mat_size, 1, []);
