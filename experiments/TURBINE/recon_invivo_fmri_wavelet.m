function [] = recon_invivo_fmri_wavelet(slices,sens_lowres, sens_highres, kdata_folder)
   ...
   %%    
    for ex = experiments
        ...
        for sl = slices
            ...
            for ws = window_sizes
                if exist([kdata_folder 'recons_nl_wavelet/Recon_sl' num2str(sl) '_ws' num2str(ws) kdata_file_a.name(1:end-12) '_iter.mat'], 'file')
                    warning(['A file named "' kdata_folder 'recons_nl_wavelet/Recon_sl' num2str(sl) '_ws' num2str(ws) kdata_file_a.name(1:end-12) '_iter.mat" already exists! no reconstruction performed'])
                    continue
                end
                ...
                W = wavelet('db2', 4);
                recon_a = reshape(fista(E, kdata_rs_a, W, 1e-6, [mat_size,mat_size,1,Nframes], 50, step, 0), mat_size, mat_size, 1, []); 

                for t = 1:size(recon_a,4)
                    recon_a(:,:,:,t) = ifft2(ifftshift(fftshift(fft2(recon_a(:,:,:,t))).*mask));
                end

                save([kdata_folder 'recons_nl_wavelet/Recon_sl' num2str(sl) '_ws' num2str(ws) kdata_file_a.name(1:end-12) '_iter.mat'], 'recon_a')
            end
        end
    end
